...
export var getChainByKey = function (chainKey) {
    var chain = supportedChains.find(function (chain) { return chain.key === chainKey; });
    if (!chain) {
        throw new Error('Invalid chainKey');
    }
    return chain;
};
export var getChainById = function (chainId) {
    var chain = supportedChains.find(function (chain) { return chain.id === chainId; });
    if (!chain) {
